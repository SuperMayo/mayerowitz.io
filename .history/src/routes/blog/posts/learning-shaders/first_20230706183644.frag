...
void main() {
	vec2 st = gl_FragCoord.xy/(resolution.xy); // normalize the coordinates
    ...
    float d4 = sdSphere(st, vec2(map(cos(time), -1., 1., 0.4, 0.7), 0.3), 0.1); 
    ...
    vec2 centroidx = (vec2(0.4, map(cos(time), -1., 1., 0.3, 0.4)) +
        ...
        vec2(map(cos(time), -1., 1., 0.4, 0.7), 0.3))/4.0;
    // Apply dithering
    float dither = bayerMatrix[int(gl_FragCoord.x/2.) % 4][int(gl_FragCoord.y/2.) % 4];
    float metadither = step(dither, distance(st, centroidx/2.));
