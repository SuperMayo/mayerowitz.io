varying vec2 vUv;
uniform float u_slider;

float circleSDF(vec2 p, float r)
{
  return length(p) - r;
}

void main() {
    vec2 uv = vUv;

    // distance of the current pixel from the center of the screen
    // 1-distance to get brighter in the center
    float d1 = circleSDF(vec2(0.5) - uv, u_slider);

    // Default color to black
    vec3 color = vec3(0.0, 0.0, 0.0);
    // Blue if d1 is negative
    color.b = smoothstep(0.01, 0., d1) * fract(d1*10.); // Red if close to d1
    // Red if d1 is positive
    color.r = smoothstep(0., 0.01, d1) * fract(d1*10.); // Green if close to d1

    // Increase luminosity with distance to center
    gl_FragColor = vec4(color, 1.0); //RGBA
}