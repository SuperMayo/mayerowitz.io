...
void main() {
  ...
  vec2 c1 = vec2(0.4,trigmap(tan(u_time), 0.3, 0.4));
  vec2 c2 = vec2(trigmap(tan(u_time), 0.4, 0.7), 0.5);
  vec2 c3 = vec2(0.5, trigmap(tan(u_time), 0.6, 0.7));
  ...
  // Background color
  float bg = sin((uv.x+uv.y)*100.*d + u_time*2.);
  bg = smoothstep(0.0, 0.1, bg);
  bg = bg / distance(ctroid*2., uv);

  vec3 bg_color = bg * vec3(
    trigmap(cos(u_time*uv.x), 0., 1.),
    trigmap(cos(u_time*uv.y), 0., 1.),
    trigmap(sin(u_time*uv.x*uv.y), 0., 1.));

  // Final color
  color = color * metaball + bg_color * (1. - metaball);
