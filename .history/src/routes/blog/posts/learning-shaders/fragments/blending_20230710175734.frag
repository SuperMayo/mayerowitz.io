...
void main() {
    ...
    float d1 = smoothstep(0.5, 0.8, sdSphere(uv, vec2(0.6), .5));
    float d2 = smoothstep(0.5, 0.8, sdSphere(uv, vec2(u_step), 0.5));

    //float color = step(0.2, d1 + d2);
    //float color  = step(0.8,smax(d1,d2,0.4));
    float color = step(0.7, d1 + d2);
