...
void main() {
    ...
    float d1 = circleSDF(vec2(0.5) - uv, 0.001);
    float d2 = circleSDF(vec2(u_slider) - uv, 0.011);

    // Set pixel color
    vec3 color = vec3(0.0, 0.0, 0.0);
    color.r = smoothstep(0.1, 0.1, d1); // Red if close to d1
    color.g = smoothstep(0., 0.1, d2); // Green if close to d2
