...
> Have you noticed the aliasing around the disk when step() is applied? This is because the step function is too abrupt. To solve this, GLSL provides a `smoothstep(float t_start, float t_end , float x)` function, which will give you... a smooth step!
