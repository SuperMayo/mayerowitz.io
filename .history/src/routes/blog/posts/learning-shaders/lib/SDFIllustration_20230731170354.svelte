<script>
    ...
    let vl = Math.sqrt(Math.pow(p.c[0] - 0.1*300, 2) + Math.pow(0.90*300 - p.c[1], 2));
    // Point of intersection between the circle and the line from the origin to the center of the circle
    let i = {
        x: p.c[0] - p.r * (p.c[0] - p.r) / vl,
        y: p.c[1] - p.r * p.c[1] / vl,
    }
...
<svg width={300} height={300} bind:this={canvas} class="font-mono m-auto">
    ...
    <line x1="10%" y1="90%" x2={i.x} y2={i.y} stroke="red" stroke-width="0.5%" marker-end="url(#arrowhead)" />
</svg>