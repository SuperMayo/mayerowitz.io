...
<svg width={300} height={300} bind:this={canvas} class="font-mono m-auto">
    ...
    {#each {length: 7} as _, i}
        <line x1={width * (0.1) + (i+1) * (width*0.1)} y1="10%"
            ...
            stroke="gray" stroke-dasharray="2,4" stroke-width="0.3%" />
        <line x1="10%" y1={height * (0.1) + (i+1) * (height*0.1)}
              x2="90%" y2={height * (0.1) + (i+1) * (height*0.1)}
              stroke="lightgray" stroke-dasharray="2,4" stroke-width="0.3%" />
    {/each}
    ...
    <!-- Circle not filled-->
    <circle cx={200} cy={100} r="40" stroke="teal" stroke-width="1%" fill="none" />
    ...
    <!-- Radius -->
    <line x1="200" y1="100" x2={200+40} y2={100} stroke="black" stroke-width="0.5%"/>
</svg>