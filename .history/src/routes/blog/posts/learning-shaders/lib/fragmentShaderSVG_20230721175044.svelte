...
<svg width={300} height={300} bind:this={canvas} class="font-mono m-auto">
    ...
    {#each {length: 7} as _, i}
        <line x1={width * (0.1) + (i+1) * (width*0.1)} y1="10%" x2={width * (0.1) + (i+1) * (width*0.1)} y2="90%" stroke="gray" stroke-dasharray="2,2" stroke-width="0.5%" />
        <line x1="10%" y1={height * (0.1) + (i+1) * (height*0.1)} x2="90%" y2={height * (0.1) + (i+1) * (height*0.1)} stroke="gray" stroke-width="0.5%" />
    {/each}
