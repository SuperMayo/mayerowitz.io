<script>
    ...
    let vl = Math.sqrt(Math.pow(p.c[0] - p.r, 2) + Math.pow(p.c[1], 2));
    // Point of intersection between the circle and the line from the origin to the center of the circle
    let i = [p.c[0] - p.r * (p.c[0] - p.r) / vl, p.c[1] - p.r * p.c[1] / vl];
...
<svg width={300} height={300} bind:this={canvas} class="font-mono m-auto">
    ...
    <line x1="10%" y1="90%" x2={p} y2={p.c[1]} stroke="black" stroke-width="0.5%" marker-end="url(#arrowhead)" />
</svg>