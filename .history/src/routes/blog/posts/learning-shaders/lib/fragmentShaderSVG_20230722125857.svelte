<script>
    ...
    onMount(() => {
        ...
        setInterval(() => {
            p[0] = width/3 + Math.cos(Date.now() / 1000) * width/5;
            p[1] = height/2 + Math.sin(Date.now() / 1000) * height/5;
        }, 10);
    });
    ...
    $: color = 'rgb(' + (normCoords(p[0]))*255 + ", " + (1 - normCoords(p[1]))*255 + ",0)";
</script>
...
<svg width={300} height={300} bind:this={canvas} class="font-mono m-auto">
    ...
    <rect x={p[0]+10} y={p[1]-5} width="43%" height="15" fill="white" />
    <rect x={p[0]} y={p[1]} width="3%" height="3%" r="2%" fill={color} />
    <text x={p[0]+10} y={p[1]+8} class="text-sm">rgba=f({normCoords(p[0])},{normCoords(p[1])})</text>
</svg>