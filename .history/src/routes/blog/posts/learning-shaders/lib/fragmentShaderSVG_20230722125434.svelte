<script>
    ...
    normCoords = (coord) => {
        return (coord - width*0.1) // shift to the left
                / (width*0.8); // normalize
    } 

    $: color = 'rgb(' + (xcoord/width)*255 + ", " + ((width-ycoord)/width)*255 + ",0)";
</script>
...
<svg width={300} height={300} bind:this={canvas} class="font-mono m-auto">
    ...
    <rect x={xcoord+10} y={ycoord-5} width="43%" height="15" fill="white" />
    <rect x={xcoord} y={ycoord} width="3%" height="3%" r="2%" fill={color} />
    <text x={xcoord+10} y={ycoord+8} class="text-sm">rgba=f({(xcoord/width).toFixed(2)},{(ycoord/width).toFixed(2)})</text>
</svg>