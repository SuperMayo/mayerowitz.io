...
<svg width={300} height={300} bind:this={canvas} class="font-mono m-auto">
    ...
    <line x1="10%" y1="260" x2={p.d[0]-5} y2={p.d[1]-5} stroke="orange" stroke-width="1%"
        stroke-dasharray="3,1"/>
    <text x={p.d[0]/2} y={p.d[1]*1.2} class="text-sm" fill="teal">d</text>
    ...
    <text x={p.c[0]/1.8} y={p.c[1]*1.8} class="text-mono">v</text>
    <!-- Radius -->
    <line x1={p.c[0]} y1={p.c[1]} x2={p.c[0] + p.r} y2={p.c[1]} stroke="black" stroke-width="0.5%" stroke-dasharray="3,1"/>
    <text x={p.c[0] + p.r/3} y={p.c[1] - 5} class="text-mono">r</text>
    <!-- Circle -->
    <circle cx={p.c[0]} cy={p.c[1]} r={p.r} stroke="teal" stroke-width="1%" fill="none" />
    <circle cx={p.c[0]} cy={p.c[1]} r="1%" fill="orange"/>
    <text x={p.c[0]-5} y={p.c[1] - 5 } class="text-sm">C</text>
</svg>