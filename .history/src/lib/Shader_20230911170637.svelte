...
<IntersectionObserver on:intersection={handleShaderIntersection}>
    <div class="flex justify-center my-one drop-shadow-md italic text-gray-500" style="pointer-events: none;">
        <ImageWrapper right_label={legend} maxWidth={String(width)}>
            <Canvas size={{width: width, height: height}}>
                {#if mouseCapture} 
                    <div on:mousemove={handleMouseMove} style="width: 100%; height: 100%; position: absolute; top: 0; left: 0; z-index: 100;"></div>
                {/if} 
