...
void main() {
    float scaleFactor = resolution.x / 1280.;
	  vec2 st = (vUv - 0.5) / scaleFactor; // Center the coordinate system (for scaling)
    float aspectRatio = (resolution.x / resolution.y);
    st.x *= aspectRatio;
    //st = floor(st * GRID_SIZE * aspectRatio) / (GRID_SIZE * aspectRatio);
    
    // Handle Mouse
    vec2 m = (vec2( mouse.x/resolution.x, 1.0 - mouse.y/resolution.y) - 0.5) / scaleFactor;// normalize mouse coordinate
    ...
    vec2 c4 = vec2(map(cos(time), -1., 1., 0.4, 0.63), 0.9);
