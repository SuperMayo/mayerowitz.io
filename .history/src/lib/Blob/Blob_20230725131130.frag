...
void main() {
    ...
    color.rgb = mix(vec3(2./255., 249./255., 240./255.),
                vec3(225./255., 230./255., 230./255.), dist);
    
    d5 = smoothstep(0.5, 0.8, d5);
    color.rgb = mix(color.rgb, vec3(242./255., 241./255., 235./255.), d5);
    // Create a noisy gradient using the noise function
    color.rgb = mix(color.rgb, vec3(255./255., 249./255., 240./255.), noise(st * 10.));
