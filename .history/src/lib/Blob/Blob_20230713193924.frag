...
void main() {
    ...
    // X fixed
    vec2 c1 = vec2(m.x, map(cos(time), -1., 1., 0.5, 0.6));
    vec2 c3 = vec2(m.x, map(sin(time), -1., 1., 0.4, 0.6));
    // Y fixed
    vec2 c2 = vec2(map(sin(time), -1., 1., 0.4, 0.7), m.y);
    vec2 c4 = vec2(map(cos(time), -1., 1., 0.4, 0.63), m.y);
