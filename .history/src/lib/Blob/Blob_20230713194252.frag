...
void main() {
    ...
    // X fixed
    vec2 c1 = vec2(map(m.x, 0., 1., 0.3, 0.4), map(cos(time), -1., 1., 0.5, 0.6));
    vec2 c3 = vec2(map(m.y, 0., 1., 0.4, 0.6), map(sin(time), -1., 1., 0.4, 0.6));
    // Y fixed
    vec2 c2 = vec2(map(sin(time), -1., 1., 0.4, 0.7), map(m.x, 0., 1., 0.4, 0.6)));
