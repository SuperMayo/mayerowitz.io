...
void main() {
    ...
    st = floor(uv * GRID_SIZE * aspectRatio) / (GRID_SIZE * aspectRatio);
    ...
    // Dithering threshold
    float dt = bayerMatrix[int(gl_FragCoord.x) % 4][int(gl_FragCoord.y) % 4];
    ...
    //gl_FragColor = vec4(230./255., 230./255., 230./255., color.a/1.);
}