...
<header class="mb-two w-full">
    <div class="flex flex-col justify-between items-center p-one max-w-screen-lg sm:flex-row">
      ...
      ${burgerOn ? 'burger-on pointer-events-auto' : 'sm:pointer-events-auto pointer-events-none'}`}>
        <nav>
          <ul class="text-right m-0 sm:flex sm:justify-start sm:items-center sm:text-left">
            {#each menuLinks as link}
              <li class="menu-item px-2 m-0 border-2">
                <a href={link.link} class="mt-quarter transition-all duration-500 ease-in-out underline"
