...
void main() {
    ...
    vec2 st = floor(gl_FragCoord.xy / resolution.xy * GRID_SIZE) / GRID_SIZE;
    ...
    vec2 c5 = clamp(vec2(0.3, tan(time/2.)), -1., 2.);
    ...
    // Dithering threshold
    float dt = bayerMatrix[int(st.x*200.) % 4][int(st.y*200.) % 4];
    float gradient = 1. - distance(st, newCenter);
    gradient = step(dt, gradient) * metaalt;
    
    // Final color
    vec4 color = vec4(0.);
    color.a = 1. - step(metaalt * dt, gradient);
    
    //gl_FragColor = vec4(vec3(dt), 1.0);
    //gl_FragColor = vec4(vec3(color.a), 1.0);
    gl_FragColor = vec4(0.0, 0.6, 0.6, color.a);
}