...
const ImageWrapper = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="flex center"><div class="${"max-w-" + escape(maxWidth, true) + " m-auto my-one italic text-gray-500"}">${slots.default ? slots.default({}) : ``}
    ${left_label || right_label ? `<div class="flex justify-between flex-row"><span class="font-mono text-left">${escape(left_label)}</span>
            <span class="font-mono text-right">${escape(right_label)}</span></div>` : ``}</div></div>`;
});
export {
  ImageWrapper as I
};
